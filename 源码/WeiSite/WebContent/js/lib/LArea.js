	...
	MobileArea.prototype = {
		...
		bindEvent : function() {
			...
			function popupArea(e) {
				...
				_self.gearArea.innerHTML = '<div class="area_ctrl slideInUp">'
						...
						+ '<div class="area_grid">' + '</div>' + '</div>'
						+ '</div>' + '</div>' + '</div>';
				document.body.appendChild(_self.gearArea);
				areaCtrlInit();
				var larea_cancel = _self.gearArea
						.querySelector(".larea_cancel");
				larea_cancel.addEventListener('touchstart', function(e) {
					_self.close(e);
				});
				var larea_finish = _self.gearArea
						.querySelector(".larea_finish");
				larea_finish.addEventListener('touchstart', function(e) {
					_self.finish(e);
				});
				var area_province = _self.gearArea
			...
			function areaCtrlInit() {
				...
				_self.gearArea.querySelector(".area_province").setAttribute(
						"val", _self.value[0]);
				_self.gearArea.querySelector(".area_city").setAttribute("val",
						_self.value[1]);
				_self.gearArea.querySelector(".area_county").setAttribute(
						"val", _self.value[2]);
			...
			function gearTouchMove(e) {
				...
				var f = (target["new_" + target.id] - target["old_" + target.id])
						* 30 / window.innerHeight;
				target["pos_" + target.id] = target["o_d_" + target.id] + f;
				target.style["-webkit-transform"] = 'translate3d(0,'
						+ target["pos_" + target.id] + 'em,0)';
			...
			function gearTouchEnd(e) {
				...
				var flag = (target["new_" + target.id] - target["old_"
						+ target.id])
						/ (target["n_t_" + target.id] - target["o_t_"
								+ target.id]);
			...
			function rollGear(target) {
				...
				target["int_" + target.id] = setInterval(function() {
					...
					target.style["-webkit-transform"] = 'translate3d(0,' + pos
							+ 'em,0)';
			...
			_self.getData(function() {
				$(_self.trigger).on("tap", popupArea)
		...
		setGearTooth : function(data) {
			...
			if (l > 0) {
				...
				var itemStr = "";
				for (var i = 0; i < l; i++) {
					itemStr += "<div class='tooth'  ref='"
							+ item[i][this.keys['id']] + "'>"
							+ item[i][this.keys['name']] + "</div>";
				}
				gearChild[_self.index].innerHTML = itemStr;
				gearChild[_self.index].style["-webkit-transform"] = 'translate3d(0,'
						+ (-gearVal * 2) + 'em,0)';
		...
		getNodeById : function(nodeId) {
			...
			switch (_self.type) {
			case 1:
				node = _self.queryNode(_self.data, nodeId, node);
				break;
			case 2:
				node = _self.queryNode(_self.data[0], nodeId, node);
				break;
			}
			return node;
		},
		queryNode : function(data, id, node) {
			var _self = this;

			for (var i = 0; i < data.length; i++) {
				if (data[i][_self.keys['id']] == id)
					node.push(data[i]);
				else {
					if (data[i].hasOwnProperty("child")) {
 						var childData = data[i].child;
						_self.queryNode(childData, id, node);
					}
				}
			}
			return node;
		...
		finish : function(e) {
			...
			var provinceCode = area_province.childNodes[provinceVal]
					.getAttribute('ref');
			...
			var countyCode = area_county.childNodes[countyVal]
					.getAttribute('ref');
			...
			if ($.isFunction(_self.change)) {
				_self.change(this.valueTo.value, _self.trigger.value);
			}
			_self.close(e);
		},
		close : function(e) {
